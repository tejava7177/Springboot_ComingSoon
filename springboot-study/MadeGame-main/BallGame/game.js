...
// 해당 document가 모두 준비되었을 때 코드들을 실행하라!
$(document).ready(function () {
    ...
    var circleTypes = {
        ...
        "small": ["black", 5, 2.5, 3000], //3000은 속도가 아니라 한 지점에서 해당 지점까지 도착하는데 걸리는 ms > 작을 수록 빠름
        "medium": ["blue", 15, 7.5, 4000],
        "large": ["yellow", 30, 15, 5000]
    };

    // circleChoice라는 변수에 "small" 값을 넣고 싶다면
    // circleType[circleChoice][0]==색
    // circleType[circleChoice][1]==크기
    // circleType[circleChoice][2]==반지름
    // circleType[circleChoice][1]==속도
    ...
    function timer() {
        // gameOn = "true";
        if (gameOn == "true") {
            // 10ms마다 function() 실행 > 10ms마다 t값이 0.01초 증가하게 되는 것
            setTimeout(function () {
                t = t + 0.01;
                // toFixed(num) : 소수 num번째 자리까지 자름
                // https://opentutorials.org/course/50/133
                //t.toFiexd(2) = 소수 2번쨰 자리까지만 찍어라
    ...
    function createCircle() {
        ...
        // body 태그에 있는 모든 객체를 대상으로 함.
        var moveableWidth = $("body").width() - circleSize;
        var moveableHeight = $("body").height() - circleSize;
        // Math.random() : 0~1
        // toFixed() : 정수로 값 반환
        // moveableWidth, moveableHeight 사이에 만들어내기 moveableHeight 보다 큰 값이 나오면 오류가 나오기 떄문에 1미만의 소수를 곱한 후 자연수처리를 한다.
        ...
        function timeCirclePosition(circleTrackID) {
            // 1ms 마다 반복실행
            setTimeout(function () {
                var currentCirclePosition = $(circleTrackID).position();    //현재 공의 좌표
    ...
    // circle 움직이기
    function animateCircle(circleName, speed, circleSize) {
        ...
        $("#" + circleName).animate({
            ...
            //반복실행
            animateCircle(circleName, speed, circleSize);
        });
    };
